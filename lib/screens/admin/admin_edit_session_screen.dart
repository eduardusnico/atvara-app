...
class _SavedLocation {
  ...
  factory _SavedLocation.fromJson(Map<String, dynamic> json) => _SavedLocation(
...
class _SavedLocationsService {
  static const _key = 'saved_locations';

  static Future<List<_SavedLocation>> load() async {
    ...
    try {
      ...
      return list
          .map((e) => _SavedLocation.fromJson(e as Map<String, dynamic>))
          .toList();
    } catch (_) {
      return [];
    }
  ...
  static Future<void> save(List<_SavedLocation> locations) async {
    final prefs = await SharedPreferences.getInstance();
  ...
  static Future<void> add(_SavedLocation location) async {
    final list = await load();
    list.add(location);
    await save(list);
  }

  static Future<void> update(_SavedLocation updated) async {
    final list = await load();
...
class AdminEditSessionScreen extends StatefulWidget {
  final String sessionId;
  const AdminEditSessionScreen({super.key, required this.sessionId});

  @override
  State<AdminEditSessionScreen> createState() => _AdminEditSessionScreenState();
}

class _AdminEditSessionScreenState extends State<AdminEditSessionScreen> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _descCtrl.text = session.description ?? '';
          _attendanceMode = session.attendanceMode;
          _startDate = session.startTime;
          _startTime = TimeOfDay.fromDateTime(session.startTime);
          _endDate = session.endTime;
          _endTime = TimeOfDay.fromDateTime(session.endTime);

          if (session.attendanceMode != AttendanceMode.online) {
            _latCtrl.text = session.targetLat.toStringAsFixed(6);
            _lngCtrl.text = session.targetLng.toStringAsFixed(6);
            _radiusCtrl.text = session.radiusMeters.toString();
          } else {
  ...
  Future<void> _loadSavedLocations() async {
    final list = await _SavedLocationsService.load();
    if (mounted) {
      setState(() {
  ...
  void _offerToSaveLocation(double lat, double lng) {
    showDialog(
      ...
      if (saved != null && mounted) {
        _SavedLocationsService.add(saved).then((_) => _loadSavedLocations());
      }
    });
  }
  ...
  void _openSavedLocations() {
    showDialog(
      ...
      builder: (ctx) => _SavedLocationsDialog(
        ...
        onDelete: (loc) async {
          await _SavedLocationsService.remove(loc.id);
        ...
        onEdit: (loc, newName) async {
          loc.name = newName;
          await _SavedLocationsService.update(loc);
          await _loadSavedLocations();
          if (ctx.mounted) Navigator.pop(ctx);
          if (mounted) {
            _openSavedLocations();
  ...
  Widget build(BuildContext context) {
    ...
    if (_fetchingSession) {
      return Scaffold(
        appBar: AppBar(title: const Text('Edit Attendance Session')),
        body: const Center(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        leading: IconButton(
          ...
          onPressed: () => context.go('/admin/sessions/${widget.sessionId}'),
        ),
      ),
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(
                    'Update the parameters for attendance check-in',
                    ...
                    children: [
                      OutlinedButton(
                        onPressed: _loading
                            ? null
                            : () => context
                                .go('/admin/sessions/${widget.sessionId}'),
...
class _SavedLocationsDialog extends StatelessWidget {
  final List<_SavedLocation> locations;
  final void Function(_SavedLocation) onSelect;
  final void Function(_SavedLocation) onDelete;
  final void Function(_SavedLocation, String) onEdit;
